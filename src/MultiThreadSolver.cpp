...
bool MultiThreadSolver::solve(Board& b) {
    std::vector<std::future<bool>> futures;

    for (int num = 1; num <= 9; ++num) {
        Board copy = b;
        if (isValid(copy, 0, 0, num)) {
            copy.set(0, 0, num);
            futures.push_back(std::async(std::launch::async, [=]() mutable {
                return solveRecursive(copy, 0, 1);
            }));
        }
    }
    ...
    for (auto& f : futures) {
        if (f.get()) {
            //b = futures[0].get(); // Optional: assign solved board
            return true;
        }
    }
    return false;
...
bool MultiThreadSolver::solveRecursive(Board b, int row, int col) {
...
bool MultiThreadSolver::isValid(const Board& b, int row, int col, int num) {
    for (int i = 0; i < 9; ++i) {
